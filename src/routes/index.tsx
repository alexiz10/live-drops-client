...
function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      {auctions.length === 0 ? (
        <div className="text-center py-20 text-zinc-500 font-medium">No auctions found for this filter.</div>
      ) : (
        <>
            ...
            {auctions.map((auction) => (
              <Link key={auction.id} to="/auctions/$auctionId" params={{ auctionId: auction.id }} className="group flex flex-col">
                <div className="relative w-full aspect-square bg-zinc-100 rounded-4xl overflow-hidden mb-5">
                  <img src={getDynamicImage(auction.id, auction.title)} alt={auction.title} className="size-full object-cover transition-transform duration-700 group-hover:scale-105" />
                ...
                <div className="flex flex-col grow px-2">
                  ...
                  <div className="mt-auto flex justify-between items-end pt-4">
                    <div>
                      <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-0.5">{status === 'ended' ? 'Final Price' : 'Current Bid'}</p>
                      <p className="text-2xl font-black text-zinc-900 tracking-tighter">{Number(auction.current_price).toLocaleString("en-US", {
                        style: 'currency',
                        currency: 'USD',
                      })}</p>
                    </div>
          ...
          {totalPages > 1 ? (
            <div className="flex justify-center items-center gap-4 mt-16">
              <button
                ...
                className="px-4 py-2 cursor-pointer font-bold text-sm bg-zinc-100 rounded-full disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </button>
              <span className="font-medium text-sm text-zinc-500">Page {page} of {totalPages}</span>
              <button
